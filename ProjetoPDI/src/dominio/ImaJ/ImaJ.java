...
public abstract class ImaJ {
    ...
    */
    ...
    public static boolean[][] im2bw_red(int[][] im_red, int[][] im_green, int[][] im_blue) {
        boolean[][] bw = new boolean[im_red.length][im_red[0].length];

        for (int i = 0; i < im_red.length; i++) {
            for (int j = 0; j < im_red[0].length; j++) {
                bw[i][j] = (im_red[i][j] > 126 && im_green[i][j] < 126 && im_blue[i][j] < 126);
            }
        }
    ...
    public static boolean[][] im2bw_black(int[][] im_red, int[][] im_green, int[][] im_blue) {
        boolean[][] bw = new boolean[im_red.length][im_red[0].length];

        for (int i = 0; i < im_red.length; i++) {
            for (int j = 0; j < im_red[0].length; j++) {
                bw[i][j] = (im_red[i][j] < 100 && im_green[i][j] < 100 && im_blue[i][j] < 100);
            }
        }
   ...
   */
    public static int[][][] _imResize(int[][][] im) {
        int[][][] image = new int[im.length / 4][im[0].length / 4][3];
        int x = 0;
        int y;
